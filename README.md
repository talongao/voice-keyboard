...
--http           强制用 HTTP（已有证书时也退回 HTTP；麦克风模式会不可用）
--quiet          不打印横幅
-h, --help       帮助
```
...
手机页顶部提供两个标签：**文字**（前述功能）与**麦克风**。切换到麦克风后，
手机采集的音频会实时传输至电脑，在会议、游戏或直播软件中选择该虚拟麦克风即可使用。

相比"手机输入法语音转文字"，该模式延迟更低、更接近原生麦克风，但使用门槛更高：
**以下两项条件均需满足**，程序会在手机页上逐步引导。

| 门槛 | 为什么 | 各平台情况 |
|---|---|---|
| ① HTTPS | 浏览器仅在安全上下文（https / localhost）下向网页开放麦克风；以 http 访问内网 IP 会被直接拒绝 | 程序生成自签证书，手机首次需手动信任（iOS：安装描述文件并在「关于本机 → 证书信任设置」中启用；Android：安装用户 CA） |
| ② 虚拟声卡 | 电脑需存在一个对会议软件可见的"麦克风"设备 | **Linux 零安装**（程序自动创建 PipeWire/Pulse null-sink）；Windows 需安装 [VB-CABLE](https://vb-audio.com/Cable/)；macOS 需安装 [BlackHole](https://existential.audio/blackhole/download/)（安装后需重启） |

协议策略：**未生成证书时全部使用 HTTP**（保持零配置体验）；**一旦为麦克风模式生成过证书，之后全部使用 HTTPS**。
如需强制退回 HTTP，使用 `--http`。

音频传输方式：手机网页通过 Web Audio 采集，每 100ms 打包一块 16-bit 单声道 PCM（48kHz）POST 至电脑。
局域网带宽约 768 kbps，且不引入任何音频编解码依赖，保持单文件与轻量。

> ⚠️ 使用期间请保持手机页面**处于前台**：息屏或切换到其他应用时，浏览器会暂停录音。
> 这是移动端浏览器的限制，原生应用不受此影响。
...
- **不录音**：麦克风模式的音频只在内存里过一手就送进虚拟声卡，不落盘、不上传
