...
手机页顶部有两个标签：**文字**（上面那些）和**麦克风**。切到麦克风后，
手机采集的声音会实时传给电脑，在会议 / 游戏 / 直播软件里选那个虚拟麦克风即可。

它比"手机输入法语音转文字"更直接，但门槛也更高——**两道门都得过**，程序会在手机页上一步步引导：

| 门槛 | 为什么 | 各平台情况 |
|---|---|---|
| ① HTTPS | 浏览器只在安全上下文（https / localhost）下才给网页麦克风，http 下的内网 IP 会被直接拒绝 | 程序生成自签证书，手机首次要信任一次（iOS：装描述文件 + 到「关于本机 → 证书信任设置」打开；Android：安装用户 CA） |
| ② 虚拟声卡 | 电脑得有一个"看起来像麦克风"的设备，会议软件才选得到 | **Linux 零安装**（程序自己建 PipeWire/Pulse 的 null-sink）；Windows 需装 [VB-CABLE](https://vb-audio.com/Cable/)；macOS 需装 [BlackHole](https://existential.audio/blackhole/download/)（装完要重启） |

协议策略很简单：**没证书就全走 HTTP**（和以前一样，零摩擦）；**一旦为了麦克风生成过证书，之后全走 HTTPS**。
想强制退回 HTTP：`--http`。

音频怎么传：手机网页用 Web Audio 采集，每 100ms 打包一块 16-bit 单声道 PCM（48kHz）POST 给电脑——
局域网约 768 kbps，不引入任何音频编解码依赖（守住单文件、轻量）。

> ⚠️ 用的时候请让手机页面**留在前台**：息屏或切到别的 App，浏览器会暂停录音（这是手机浏览器的限制，
> 原生 App 才能后台跑）。
